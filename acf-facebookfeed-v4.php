...
class acf_field_facebookfield extends acf_field {
  ...
  <tr class="field_option field_option_<?php echo $this->name; ?>">
    <td class="label">
      <label><?php _e("Endpoint",'acf'); ?></label>
			<span class="sub-field-instructions"><a href="https://developers.facebook.com/docs/graph-api/reference/v2.0/">https://graph.facebook.com/v2.0/</a></span>
    </td>
    <td>
      <?php
      do_action('acf/create_field', array(
	...
	*/
	...
	function create_field( $field )
	{
		...
		*/
		...
		$username      				= ( isset($field['value']['username'])) ? $field['value']['username'] : '';
    $endpoint      				= ( isset($field['value']['endpoint'])) ? $field['value']['endpoint'] : '';
		$cache_expiration      = ( isset($field['value']['cache_expiration'])) ? $field['value']['cache_expiration'] : 60;

    ?>
    <table id="acffacebook" class="widefat acf-input-table row_layout">
      <tbody>
        <tr>
    ...
    <table class="widefat acf_input">
		...
		<tr class="field">
			...
			<td>
				<input type="text" value="<?php echo $username; ?>" id="<?php echo $key.'[username]'; ?>" name="<?php echo $key.'[username]'; ?>">
			</td>
		</tr>
    ...
    <tr class="field">
      ...
      <td>
        <input type="text" value="<?php echo $endpoint; ?>" id="<?php echo $key.'[endpoint]'; ?>" name="<?php echo $key.'[endpoint]'; ?>">
      </td>
    </tr>
	...
	*/
	...
	function input_admin_enqueue_scripts()
	{
		...
		wp_register_script( 'acf-input-facebookfield', $this->settings['dir'] . 'js/input.js', array('acf-input'), $this->settings['version'] );
		wp_register_style( 'acf-input-facebookfield', $this->settings['dir'] . 'css/input.css', array('acf-input'), $this->settings['version'] );


		// scripts
		wp_enqueue_script(array(
			'acf-input-facebookfield',
		));
  ...
  function update_field( $field, $post_id )
  {
	...
	*/
	...
	public function make_api_call( $endpoint, $cache_expiration = 60 ){
		...
		$transient_key = substr('facebook' . sha1( $endpoint ), 0, 32); // restrain key length otherwise expiration isn't set

		if ( false === ( $converted_response = get_transient( $transient_key ) ) ) {

			if( !get_option('acf_facebook') ) {
				return 'Missing Facebook options';
			}

			$api_keys 		 		= get_option('acf_facebook');
			$api_credentials  = 'access_token=' . $api_keys['facebook_app_id'] . '|' .  $api_keys['facebook_app_secret'];

			$api_url = 'https://graph.facebook.com/v2.0/' . $endpoint;
			...
			if( acf_field_facebookfield::json_invalid($raw_response) !== false ){
				return acf_field_facebookfield::json_invalid($raw_response);
			}
